...
#     majority_df = df[df[feature].between(min_age, max_age,
#                                          inclusive='both')]  # df containing people aged between min_age and max_age
#     minority_df = df[df.isin(majority_df) == False].dropna()
#     print(len(majority_df) + len(minority_df))
#
#     # Upsample the majority class
#     majority_upsampled = resample(majority_df, replace=True, n_samples=int(len(majority_df) * sampling_factor),
#                                   random_state=42)
...
#
#     normalized_weights = {key: ((v - min_proportion) / (max_proportion - min_proportion)) for (key, v) in
#                           address_weights.items()}
