...
def mutate(seed, session):
    ...
    while fitness > 0.50:
        ...
        #feature_index = seed[0].columns.get_loc("persoon_leeftijd_bij_onderzoek")
...
def mutate_2(seed, session, threshold=0.2, retrace_features=None): 
    # This version only stores large differences in fitness
    # retrace_features can supply a list of features the other model modified
    ...
    while fitness > 0.50:
        ...
        # Store feature column indices that caused large differences in fitness
        difference = abs(prev_fitness - fitness)
        if difference >= threshold: 
            features_list.append(feature_index)
            fitness_list.append(difference)
        iteration += 1
    #fitness_dict[iteration] = fitness_list
    fitness_feature_dict = dict(zip(features_list, [fitness_list]))
    return fitness_feature_dict
...
def visualize_influential_features_2(dict, data, top_n, model_name): # model_name is 'Model 1' or 'Model 2'
    # Count occurences of each feature that passed threshold along with effect on fitness
    dict = {feature: (len(differences), mean(differences)) for feature, differences in dict.items()}
...
def test():
    ...
    random_samples = X.sample(n=5, axis=0)

    # for multiple samples, add different fitness values to list of already chosen features in dict
    for sample in random_samples:
        original_data = sample.values.reshape(1, -1).astype(np.float32)
        ...
        for i in range(1, num_iterations+1):
            model1_fitness_list = mutate(seed, session) 
            model1_fitness_feature_dict[i] = model1_fitness_list
            model2_fitness_list = mutate(seed, session_2)
...
def test_2():
    ...
    data = pd.read_csv(DATASET_PATH)
    X = data.drop(columns=['Ja', 'Nee', 'checked'])
    random_samples = X.sample(frac=0.001, axis=0)
    #print(random_samples.iloc[0])
    #random_features = X.sample(n=5, axis=1)

    # Use defaultdict to automatically initialize lists for each key
    model1_fitness_feature_dict = defaultdict(list)
    model2_fitness_feature_dict = defaultdict(list)

    for i in range(len(random_samples)):
        original_data = random_samples.iloc[i].values.reshape(1, -1).astype(np.float32)
        ...
        num_iterations = 10
        for i in range(1, num_iterations + 1):
            model1_fitness_list = mutate_2(seed, session, threshold=0.5)  # This returns a dictionary
            model2_fitness_list = mutate_2(seed, session_2, threshold=0.5)  # This also returns a dictionary

            # Merge model1_fitness_list into model1_fitness_feature_dict
            for key, value in model1_fitness_list.items():
                model1_fitness_feature_dict[key].extend(value)

            # Merge model2_fitness_list into model2_fitness_feature_dict
            for key, value in model2_fitness_list.items():
                model2_fitness_feature_dict[key].extend(value)

    # Optionally convert defaultdict back to a regular dict if needed
    model1_fitness_feature_dict = dict(model1_fitness_feature_dict)
    model2_fitness_feature_dict = dict(model2_fitness_feature_dict)

    # Return or use the merged dictionaries as needed
    return model1_fitness_feature_dict, model2_fitness_feature_dict


if __name__ == "__main__":
    num_features = num_features
    DATASET_PATH = 'data/investigation_train_large_checked.csv'
    data = pd.read_csv(DATASET_PATH)
    model_1, model_2 = test_2()
    print("model 1: ", model_1)
    print("model 2: ", model_2)
    visualize_influential_features(dict=model_1, data=data, top_n=5, model_name="Model 1")
    visualize_influential_features(dict=model_2, data=data, top_n=5, model_name="Model 2")
    ...
    DATASET_PATH = 'data/investigation_train_large_checked.csv'  # original file from Brightspace, no changes
    data = pd.read_csv(DATASET_PATH)
    print(data.columns.get_loc("persoon_leeftijd_bij_onderzoek"))
    X = data.drop(columns=['Ja', 'Nee', 'checked'])
    ...
    num_iterations = 5 # I think fix to 50? for consistency
    model1_fitness_feature_dict = {}
    model2_fitness_feature_dict = {}
    for i in range(1, num_iterations+1):
        model1_fitness_list = mutate(original_data, session)
        print(model1_fitness_list)
        model1_fitness_feature_dict[i] = model1_fitness_list
    ...
    #visualize_influential_features(dict=major_differences, data=X, top_n=10, model_name="Model 1")

    #major_differences_2 = find_influential_features(model2_fitness_feature_dict, session_2, 0.2)
    #visualize_influential_features(dict=major_differences_2, data=X, top_n=10, model_name="Model 2")
